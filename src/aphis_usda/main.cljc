(ns aphis-usda.main
  "Kotodama WASM entrypoint for the aphis_usda clean-room actor (L4) — Clojure port.
  ...
  facts (`aphis_usda.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "aphis_usda")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"aphis_usda.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "aphis_usda-compat" :tier tier :entities entities} 200])
...
(defn start! [] :aphis_usda-compat/ready)
